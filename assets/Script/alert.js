...
let show = function (tipStr,rightStr,callback) {
    cc.loader.loadRes("alert2-min",cc.Prefab, function (error, prefab){
        if (error){
            cc.error(error)
            return
        }
        ...
        cc.find("tipAlert/title").getComponent(cc.Label).string = tipStr
        // cc.find("tipAlert/content/bottom/left_btn/leftbtn").getComponent(cc.Label).string = leftStr
        cc.find("tipAlert/button/Background/Label").getComponent(cc.Label).string = rightStr
        if(callback){
            // cc.find("tipAlert/content/bottom/left_btn").on('click', function (event) {
            //     dismiss()
            //     callback(leftStr)
            // }, this)
            cc.find("tipAlert/button/Background/Label").on('click', function (event) {
                dismiss();
                callback(rightStr)
            }, this)
        }
